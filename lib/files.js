...
exports.listFilesInDirectory = exports.getFile = exports.saveFilesInDirectory = void 0;
 ...
 */
const saveFilesInDirectory = async (ipfsClient, files) => {
  // Infura client is the only one available that allows the upload of files
  // TODO add support for mime types
  const arrayOfFiles = await ipfsClient.add(files, {
    wrapWithDirectory: true
  });
  const directoryIndex = arrayOfFiles.findIndex(({
    path
  }) => path === '');
...
exports.saveFilesInDirectory = saveFilesInDirectory;

const getFile = (ipfsClient, cid) => ipfsClient.get(cid); // Infura and ipfs.io clients allow file download
...
const listFilesInDirectory = (ipfsClient, cid) => ipfsClient.ls(cid); // ipfs.io client is the only one available that allows the listing of directories
