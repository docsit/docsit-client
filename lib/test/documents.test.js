...
var _ipfs2 = _interopRequireDefault(require("../ipfs"));
...
describe('Describe document module', () => {
  ...
  before(() => {
    ...
    context.contract = (0, _contract2.default)(context.web3);
    context.ipfsClient = (0, _ipfs2.default)();
  });
  it('gets unencrypted document with no options', async () => {
    const documentData = await (0, _documents.getDocument)(context.contract, context.ipfsClient, 'a8719f44df881b7c');
    (0, _chai.expect)(documentData).to.eql({
      id: 'a8719f44df881b7c',
  ...
  it('gets unencrypted document with organization details', async () => {
    ...
    (0, _chai.expect)(documentData).to.eql({
      ...
      organization: {
        ...
        documentIds: ['1eff3b7333183499', '7f16f3668b9efa6f', 'a4e95e3b90d4fcec', 'fef18a53e7282b83', 'c92a0af21d8c85c4', '221fb2db2b30e17b', 'c6bd5503e8a17cf0', '221fb2db2b30e17b', 'a3fc220059d90999', 'a8719f44df881b7c', '34206c975e2e09e7', 'c0d736e9dcca10ee', '707d221731bdf977', '1ecd7cabbce08050', '1f0bf9f73d4267cf', '180df03800d8bb0c', 'c9160ead4dc366a9', '150e214c9b682ce6', 'b4d667afdd227fe5', '102229f2198659b3', '36b35eca24d74d01', 'ebde1248df2e4a0a', 'c7a044e63732ae72', '33f04d11edd6a98f', 'fdba6be4dfd22a79', 'f63f4631526973e1', '15523e2175668aa3', 'c2a040b6bcf5705e', '45653c51de33a2ef', '02c664fd182ab388', '0e6a3b7ad38c3dfe', 'e2bb559443cd7a47', '77c7da1516b12f36', '4d3d570a373bf5b5', '31003f53970db2b4', 'd662f6c63e629459'],
        id: '1fd5f224b87d03f4',
  ...
  it.skip('gets unencrypted document with attachmented files', async () => {
    const documentData = await (0, _documents.getDocument)(context.contract, context.ipfsClient, 'd47b0bc94ddde188', {
      attachedFiles: true
    });
    (0, _chai.expect)(documentData).to.eql({
      id: 'd47b0bc94ddde188',
